...
<main class="">
    ...
    >
        ...
        >
            ...
            {:else}
                <button
                    class="text-white bg-indigo-500 border-0 py-2 px-8 rounded text-lg"
                    disabled
                >
                    Log in
                </button>
            {/if}
            <p class="text-xs mt-3 flex justify-center items-center w-full">
                Don't have an account?&nbsp<a href=".#/register"
                    ><span class="text-nord8"> Join now</span></a
                >!
            </p>
        </div>
    </form>
